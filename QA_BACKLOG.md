...
- ✅ **B-18. 확정 세트라 "문항별 시간 설정"이 어떤 방에서도 저장되지 않는다** — **해결(2026-09-08)**.
  백엔드가 방 단위 오버라이드 `GET/PUT /rooms/{roomId}/question-times`를 열었고(2026-09-07 결정 (b),
  확정 세트는 불변 유지), 화면은 세트 문항 수정 대신 이 API로 읽고 전체 교체로 저장한다.
  잠금은 세트 확정 여부가 아니라 **방 시작 여부**로 건다(대기 중에만 저장, 409 `CONFLICT`).
- ✅ **B-19. 자동 넘김(`autoAdvance`)을 담을 필드가 없다** (= D-15) — **해결(2026-09-08)**.
  `PUT /rooms/{roomId}/question-times`의 문항 항목에 `autoAdvance`가 생겼다(방 단위, 2026-09-07).
  토글을 풀고 시간과 함께 저장한다 — 시간 만료로 마감된 문항만 서버가 다음 문항을 자동으로 연다.
...
- ✅ **B-21. 호스트용 방 상세에 문항 수·호스트 이름이 없다** — **해결(2026-09-08)**. `RoomResponse`에
  `host`·`questionCount`·`estimatedSeconds`·`min/maxTimeLimitSec`가 생겼다(2026-09-07). 대기실이 세트
  상세를 더 읽지 않고 방 상세로 그린다 — 제한시간 범위는 방이 덮어쓴 시간(W-02b)이 반영된 값이다.
