...
| `NEXT_PUBLIC_ENABLE_DEV_LOGIN` | 로그인 화면에 개발용 로그인(`POST /auth/dev-login`) 칸을 띄운다. **`1`일 때만** 켜진다. 이 API는 백엔드 local·dev 프로파일에만 있으므로 로컬 백엔드에 붙일 때만 켠다 — CI 배포는 브랜치와 무관하게 늘 끈다 |
