  ...
  return (
    ...
    >
      ...
      <td className="text-label-md text-muted-foreground">
        {row.correctCount === null ? "미제출" : `${row.correctCount}/${questionCount}`}
      </td>
      <td className="pr-[18px] text-right text-label-lg text-ink">
        {row.correctCount === null ? "—" : `${formatNumber(row.score)}점`}
      </td>
    </tr>
  );
}
