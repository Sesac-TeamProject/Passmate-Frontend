...
export function HostDashboard() {
  return (
    <main className="flex flex-col gap-6 px-9 py-7">
      <WelcomeBanner name={ACCOUNT.name.slice(1)} createHref="/host/rooms/new" />
      <StatCards stats={DASHBOARD_STATS} />
      <QuestionSetCards sets={QUESTION_SETS.slice(0, 3)} allHref="/host/sets" />
      <RecentSessions
        sessions={PAST_SESSIONS}
        allHref="/host/sessions/1/review"
        reportHref={(s) => `/host/sessions/${s.id}/review`}
      />
    </main>
  );
}
