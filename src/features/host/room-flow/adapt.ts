...
import type { QuestionSetSummaryResponse, RoomCreateRequest } from "@/lib/types/dto";
...
/**
 * GET /question-sets?status=CONFIRMED → 세트 선택 옵션.
 * 확정 세트만 방에 붙일 수 있다 — 초안을 고르면 서버가 세션 시작에서 막는다.
 */
export function toQuestionSetOptions(sets: QuestionSetSummaryResponse[]): QuestionSetOption[] {
  return sets.map((s) => ({
    id: String(s.id),
    title: s.title,
...
export function isFormLevelCreateError(error: unknown): boolean {
  ...
  return (
    ...
    error.code === ERROR_CODES.QUESTION_SET_NOT_FOUND ||
    error.code === ERROR_CODES.INSUFFICIENT_COINS
  );
}

/** 방 설정 한 줄 요약 (W-02e "입력한 설정은 그대로 남아 있어요" 아래 줄) */
export function toRoomSummary(body: RoomCreateRequest, sets: QuestionSetOption[]): string {
  const set = sets.find((s) => s.id === String(body.questionSetId));
  const parts = [
    body.title,
    set ? `${set.title} ${set.questionCount}문항` : null,
    body.type === "PAID" ? "유료" : "무료",
  ];
  return parts.filter(Boolean).join(" · ");
}

/** 보낸 값 → 폼 초기값. W-02e에서 "설정으로 돌아가기"로 돌아올 때 입력을 되살린다 */
export function toNewRoomInitialValues(body: RoomCreateRequest): NewRoomInitialValues {
  return {
    title: body.title,
    setId: body.questionSetId != null ? String(body.questionSetId) : "",
    roomType: body.type === "PAID" ? "paid" : "free",
    fee: body.fee ?? DEFAULT_ENTRY_FEE,
  };
}
...
export function toCreateRoomErrorMessage(error: unknown): string {
  ...
  // 서버는 유료·브랜디드 방을 아직 열지 않는다 — 등급과 무관하게 400으로 막힌다
  if (error.code === ERROR_CODES.UNSUPPORTED_ROOM_TYPE)
    return "유료 방은 아직 준비 중이에요. 무료 방으로 열어 주세요";
  if (error.code === ERROR_CODES.QUESTION_SET_NOT_FOUND)
    return "문제 세트를 찾을 수 없어요. 목록을 새로고침해 주세요";
