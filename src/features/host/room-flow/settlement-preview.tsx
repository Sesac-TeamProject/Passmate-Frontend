...
export function SettlementPreview({ fee, hostShare }: Props) {
  const host = Math.round(fee * hostShare);
  const platform = fee - host;
  const hostPct = Math.round(hostShare * 100);
  ...
  return (
    <div className="flex w-full flex-col gap-1.5 rounded-2xl bg-muted px-[18px] py-3">
      ...
      <div className="flex items-center justify-between gap-3">
        <span className="text-label-lg text-ink">선생님 정산 ({hostPct}%)</span>
        <span className="text-heading-sm text-mint-dark">{won(host)}</span>
      </div>
      <div className="flex items-center justify-between gap-3">
        <span className="text-body-md text-muted-foreground">플랫폼 수수료 ({100 - hostPct}%)</span>
