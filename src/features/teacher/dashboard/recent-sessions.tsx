...
import type { PastSession } from "@/features/teacher/mock";
      ...
      <ul className="rounded-[20px] border bg-card px-2 py-1">
        {sessions.map((s) => (
          <li key={s.id} className="flex items-center gap-4 border-b-2 px-4 py-3.5 last:border-b-0">
            <span className="flex h-10 w-[54px] shrink-0 items-center justify-center rounded-xl bg-muted text-[13px] font-black text-mint-dark">
              {s.date}
            </span>
            ...
            <Link
              href={reportHref(s)}
              className="flex h-[38px] shrink-0 items-center rounded-xl bg-muted px-[18px] text-[13px] font-bold text-mint-dark transition-colors hover:bg-mint-tint"
            >
              리포트 보기
            </Link>
          </li>
