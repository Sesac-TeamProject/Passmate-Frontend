...
커밋 전 `pnpm format && pnpm lint && pnpm test && pnpm build`. 라우트를 건드렸으면 `pnpm check:routes`까지 — 48개(라우트 43 + 리다이렉트 4 + `/dev`)를 기대한다.
...
PC(768px 이상)는 바꾸지 않는다. 폰 변경은 `max-md:`·`md:hidden` 으로만 한다.
앱(KMP) 시안과 다를 때는 **"이 차이가 좁은 화면 때문인가, 앱이라는 플랫폼 때문인가"** 로 가른다.

- **맞춘다 — 좁은 화면이 만든 차이**
  - 배치: 열 수 · 쌓는 순서 · 카드 폭 · 여백 (예: 뱃지 8개 한 줄 → 4열)
  - 폰 전용 셸: 상단바 · 하단 탭바 · 바텀시트 — 가운데 모달은 390px 에서 잘린다
  - 페이지 배경: PC 의 회색(`bg-background`) 위 흰 카드 구조가 폰에서는 성립하지 않는다 —
    카드가 폭을 꽉 채워 회색이 좌우 띠로만 남는다. 폰은 `max-md:bg-card` + 구분선
- **맞춘다 — 디자인 시스템**
  - 색 · 글자 크기 · 간격 · 모서리 토큰, 공용 아이콘(`icon/*`)
  - 앱에 맞추는 게 아니라 하나의 시스템을 따르는 것이라 **PC 까지 함께 바뀌어도 맞다**.
    다만 PC 가 함께 바뀌는 변경은 먼저 알리고 진행한다
- **안 맞춘다 — 기본값은 웹**
  - 문구 · 카피 (예: "30일 연속 활동"(앱) ↔ "30일 연속"(웹))
  - 기능 유무 · 어떤 지표를 보여줄지
  - 이유 — 맞추기 시작하면 웹이 앱의 열화판이 되고, PC · 폰이 서로 다른 내용을 들면
    유지보수 지점이 두 배가 된다

**멈추고 묻는 자리**: 시안이 폰에서 정보를 빼거나 더한 경우(예: M-09 에 레벨 사다리 없음)는
디자인이 아니라 기획이다. 임의로 정하지 않는다.

**비용으로 한 번 더 거른다**
- `max-md:` 몇 줄로 끝나면 그냥 맞춘다 — 되돌리기 싸다
- 마크업을 **두 벌** 만들어야 하면 멈추고 묻는다. 한 벌이 늘 때마다 한쪽만 고치는 사고가
  따라붙는다 (이미 `ProfileCard` · `LevelLadder`/`MobileLevelCard` 가 두 벌)
...
- **값이 비어 올 수 있는 자리** — `0`·`Lv.1`로 채우지 말고 UI를 감춘다(있지도 않은 사실을 만든다).
  - 등급·뱃지·평균 별점은 이제 **서버가 계산한다** — `GET /users/me/grade`가 `level`·`levelName`·`avgRating`·승급 진행도를 준다(2026-09-04 실서버 확인). 비는 건 방 목록·공개 프로필에 실리는 요약 쪽이다.
  - 유료 방 개설은 여전히 프런트가 잠그지 않는다. 서버의 403 `HOST_LEVEL_REQUIRED`가 판정한다(실서버 확인).
